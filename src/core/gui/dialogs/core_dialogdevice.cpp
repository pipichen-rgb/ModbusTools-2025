...
void mbCoreDialogDevice::initializeBaseUi()
{
    ...
    for (int i = 1 ; i < ls.count(); i++) // pass 'DefaultSwapData' for device
...
MBSETTINGS mbCoreDialogDevice::cachedSettings() const
{
    ...
    m[prefix+vs.swapBytes                ] = m_ui.cmbSwapBytes               ->currentText(); // TODO: Default order special processing
...
void mbCoreDialogDevice::setCachedSettings(const MBSETTINGS &m)
{
    ...
    it = m.find(prefix+vs.swapBytes                ); if (it != end) m_ui.cmbSwapBytes               ->setCurrentText(it.value().toString());
...
void mbCoreDialogDevice::fillForm(const MBSETTINGS &m)
{
    ...
    it = m.find(vs.swapBytes                ); if (it != end) m_ui.cmbSwapBytes               ->setCurrentText(it.value().toString());
...
void mbCoreDialogDevice::fillData(MBSETTINGS &m) const
{
    ...
    m[vs.swapBytes                ] = m_ui.cmbSwapBytes               ->currentText (); // TODO: Default order special processing
