...
QStringList saveSendMessages(const QList<const mbClientSendMessageParams *> messages)
{
    ...
    {
        res.append(saveSendMessageParams(*p));
    }
    return res;
}
...
QList<const mbClientSendMessageParams *> restoreSendMessages(const QStringList &messages)
{
    ...
    {
        auto ptr = restoreSendMessageParams(p);
        if (ptr)
            res.append(ptr);
    }
    return res;
...
QString saveSendMessageParams(const mbClientSendMessageParams &params, bool useFunc)
{
    QString res;
    switch(params.func)
    {
    ...
    case MBF_DIAGNOSTICS:
        res = QString("subfunc=%1;format=%2;data=%3").arg(params.subfunc).arg(mb::enumFormatKey(params.format)).arg(params.data);
        break;
    ...
    case MBF_READ_WRITE_MULTIPLE_REGISTERS:
        res += QString("readoffset=%1;readcount=%2;readformat=%3;writeoffset=%4;writecount=%5;writeformat=%6;data=%7")
               ...
               .arg(mb::enumFormatKey(params.writeFormat))
               .arg(params.data);
        break;
...
mbClientSendMessageParams* restoreSendMessageParams(const QString &params, uint8_t func)
{
    QHash<QString, QString> map = getStringParamMap(params);
    if (func == 0)
    {
        bool ok = false;
        func = static_cast<uint8_t>(map.value(QStringLiteral("func")).toInt(&ok));
        if (!ok)
            return nullptr;
    }
